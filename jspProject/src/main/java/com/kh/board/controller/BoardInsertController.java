...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//일반 방식이 아닌 multipart/form-data로 전송하는 경우 request로 부터 값 추출이 불가하다
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			...
			/*
			 * 2. 전달된파일의 파일명 수정 및 서버에 업로드 작업(폴더에 저장)
			 * 
			 * >> HttpServletRequest request = > MultipartRequest multiRequest 변환
			 * 
			 * 위 구문 한줄 실행만으로 넘어온 첨부파일이 해당폴더에 무조건 업로드 된다.
			 * 단, 업로드시 파일명을 수정해주는게 일반적이다. 그래서 파일명을 수정해주는 객체를 제시해야함
			 * => 같은 파일명이 존재하는 경우 덮어씌워질 수 있고, 파일명에 한글/특수문자/띄어쓰기가 포함 경우 서버에 따라 문제발생
			 * 
			...
			Attachment at = null; // null로 초기화 후에 넘어온 첨부파일이 있다면 생성
